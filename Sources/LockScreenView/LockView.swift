 ...
 * - Abstract: A view that displays a login screen with options for entering
 *             a master password or using biometric authentication
 * - Description: Add `MainView` to this `LockScreenView` (when app goes to
 *                background mode, lock-screen covers main-view etc)
...
public struct LockView<Content: View, LockScreen: View>: View {
    ...
    * - Description: This property controls the display of the lock screen
    *                based on the current authentication state, ensuring secure
    *                access to the application's main content.
    */
   internal let lockScreen: LockScreen
   /**
    * Listen for scene change (tracks inactive, active, background)
    * - Description: This callback is responsible for handling changes in the
    *                app's scene phase, such as transitioning between active,
    *                inactive, and background states. It updates the
    *                authentication configuration based on the new scene phase
    ...
    *                of the current state of the app's scene. It can be in
    *                one of three states: `active`, `inactive`, or `background`.
    *                This is used to manage the behavior of the app based on
    *                its state, such as locking the screen when the app goes
    *                to the background.
   ...
   /**
    * init
    * - Description: This initializer creates an instance of LockView,
    *                which is a wrapper for the main content of the application.
    *                It provides a lock screen functionality that handles the
    *                authentication process and manages the transitions between
    *                different authentication states. The content to be displayed
    *                when the view is unlocked is passed as a closure parameter
    *                to this initializer.
    * - Parameters:
    *   - content: The content to be displayed when the view is unlocked.
    *              This can be any SwiftUI view that represents the app's main
    *              content.
    ...
    *   - onScenePhaseChange: This call back might not be needed as we can use
    *                         the environment variable to detect when the app
    *                         goes to the background or foreground in any other
    *                         place as well
